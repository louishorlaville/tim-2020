...
function getClasses(session) {
  ...
  for (i = 0; i < tableRows; i++) {
    ...
    if (classList[currentClass] != null) {
      ...
      document.querySelector("#session-row-" + (currentClass + 1)).innerHTML +=
        `
      <th class="class-cell fadein-chem" style="animation-delay: ` +
        (i / 20 + session / 4) +
        `s"><span class="class-header ` +
        headerColor +
        `">` +
        classList[currentClass].classID +
        `</span><div class="class-content color-bars">` +
  ...
  for (i = 0; i < classList.length; i++) {
    ...
    document.querySelector("#cheminement-table-mobile").innerHTML +=
      `
    <tr><th class="class-container-mobile fadein-chem" style="animation-delay: ` +
      (i / 20 + session / 4) +
